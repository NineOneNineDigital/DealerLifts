...
4. Under **Callback URI(s)** add:
   - `http://localhost:3000/api/auth/shopify/callback` (local dev)
   - `https://dealerlifts.com/api/auth/shopify/callback` (production)
   - Any Vercel preview URL patterns if used, e.g. `https://dealerlifts-*.vercel.app/api/auth/shopify/callback`
...
```bash
SHOPIFY_CUSTOMER_ACCOUNT_API_CLIENT_ID=<paste client id>
SHOPIFY_CUSTOMER_ACCOUNT_API_URL=https://shopify.com/<store-id>/account/customer/api/<version>
SHOPIFY_CUSTOMER_ACCOUNT_AUTH_URL=https://shopify.com/authentication/<store-id>
# The OAuth redirect origin is derived from NEXT_PUBLIC_SITE_URL —
# set that to the prod URL on Vercel (e.g. https://dealerlifts.com).
...
2. Navigate to `/api/auth/shopify/login` — you should be redirected to Shopify's login page.
3. Sign in (or create a test account).
4. You should be redirected back to `/api/auth/shopify/callback` and then to `/account`.
5. Navigating to `/account` should show customer data (name, email, orders).

## 6. Production checklist

- [ ] `NEXT_PUBLIC_SITE_URL` set to `https://dealerlifts.com` (or the Vercel URL) — this is the OAuth redirect origin.
- [ ] Callback URIs in Shopify Admin updated to include the production domain (`/api/auth/shopify/callback`).
- [ ] JavaScript origins updated to include the production domain.
- [ ] `SHOPIFY_CUSTOMER_ACCOUNT_API_CLIENT_ID`, `SHOPIFY_CUSTOMER_ACCOUNT_API_URL`, and `SHOPIFY_CUSTOMER_ACCOUNT_AUTH_URL` set in Vercel env vars.
...
| "invalid_client" from Shopify | Wrong `client_id` — double-check `SHOPIFY_CUSTOMER_ACCOUNT_API_CLIENT_ID` |
